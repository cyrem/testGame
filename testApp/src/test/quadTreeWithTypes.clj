(ns test.quadTreeWithTypes
  (:require [clojure.zip :as zip]
            [test.dataTypesMatrix :refer (->XYPoint) ]
            [test.dataTypesMatrix :refer (->Rectangle) ])
  (:use [clojure.core.match :only (match)]
        [test.dataTypesMatrix])
  (:import [test.dataTypesMatrix] ))
...
(deftype QuadNode [^int d ^int md ^clojure.lang.PersistentVector ^Rectangle b c ^clojure.lang.PersistentVector ^volatile-mutable o]
  IQuadNode
...
(defn buildQuadTree [^QuadNode node]
  (cond (< (.d ^QuadNode node) (.md ^QuadNode node)) 
        (let [splitted (split ^Rectangle (.b node))
              newDepth (inc (.d node))]
          (->QuadNode   (.d ^QuadNode node) (.md ^QuadNode node) (.b ^QuadNode node) [(buildQuadTree (->QuadNode newDepth (.md ^QuadNode node) (nth splitted 0) [] []))
